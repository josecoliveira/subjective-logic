...
def cumulative_fusion(CC, epistemic=True) -> HyperopinionInterface:
    ...
    if all(C.u != 0 for C in CC):
        ...
        if all(C.u == 1 for C in CC):
            ...
            for x in range(k):
                anum = np.sum(
                    [C.a[x] * np.prod([Cj.u for Cj in CC if Cj != C]) for C in CC]
                ) - np.sum([C.a[x] for C in CC]) * np.prod([C.u for C in CC])
                adem = np.sum(
                    [np.prod([Cj.u for Cj in CC if Cj != C]) for C in CC]
                ) - N * np.prod([C.u for C in CC])
...
def weighted_fusion(CC, epistemic=True):
    ...
    if all(C.u != 0 for C in CC) and any(C.u != 1 for C in CC):
        b = np.array([])
        dem = np.sum(
            [np.prod([Ci.u for Ci in CC if Ci != C]) for C in CC]
        ) - N * np.prod([C.u for C in CC])
        for x in range(kappa):
            num = np.sum(
                [
                    C.b[x] * (1 - C.u) * np.prod([Ci.u for Ci in CC if Ci != C])
                    for C in CC
                ]
            )
            b = np.append(b, num / dem)
        ...
        a = np.array([])
        for x in range(k):
            ax = np.sum([C.a[x] * (1 - C.u) for C in CC]) / (
                N - np.sum([C.u for C in CC])
            )
            a = np.append(a, ax)
