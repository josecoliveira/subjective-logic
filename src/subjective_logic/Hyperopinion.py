...
class Hyperopinion(HyperopinionInterface):
    ...
    def relative_a(self, x: Union[int, set], xi: Union[int, set]):
        ...
        if x.intersection(xi) == set():
            return 0
        return (
            self.a[self.set_to_index(x.intersection(xi))]
            / self.a[self.set_to_index(xi)]
        )
    ...
    def __init__(self, k, b, a=None):
        ...
        if len(b) == self.kappa:
            ...
            raise Exception(
                "Size of belief mass distribution and size of domain (k) are not compatible."
            )

        if sum(self.b) > 1:
            raise Exception(
                "Sum of belief mass distribution is greater than 1. It should be equal or less than 1."
            )
        ...
        if np.sum(a) != 1:
            raise Exception(
                "Sum of base rate distribution is not 1. It should be equal to 1."
            )
